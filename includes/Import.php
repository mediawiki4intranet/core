...
class FakeUser extends User {
	...
	function load( $flags = User::READ_NORMAL ) {
	}
	function getId() {
		return 0;
	}
...
class WikiImporter {
	 ...
	 * @throws Exception
	 */
	function __construct( StubDumpArchive $archive, Config $config = null ) {
		...
		$this->reader->open( $this->mArchive->getMainPart(), NULL, LIBXML_PARSEHUGE );
	...
	function lastLocalRevision( $title ) {
		...
		$res = $dbr->select(
			...
			'Revision::fetchRow',
			array( 'LIMIT' => 1,
			       'ORDER BY' => '(CASE WHEN rev_parent_id=0 THEN 1 ELSE 0 END) DESC, rev_timestamp DESC' ) );
		$row = $res->fetchObject();
		$res->free();
		if ( $row ) {
	...
	private function processUpload( $pageInfo, $uploadInfo ) {
		...
		} elseif ( isset( $uploadInfo['fileSrc'] ) ) {
		...
		$r = call_user_func( $this->mUploadCallback, $revision );
		if ( $release ) {
			$this->mArchive->releaseBinary( $uploadInfo['src'] );
		}
		return $r;
	}
...
class WikiRevision {
	...
	function importOldRevision() {
		...
		$prevRev = $dbw->selectRow( 'revision', '*',
			array( 'rev_page' => $pageId, "rev_timestamp < ".$dbw->addQuotes( $dbTimestamp ) ), __METHOD__,
	...
	function importUpload() {
		...
		if ( !$file ) {
			// May fail if a target namespace or root page is passed
			return false;
		}
		$archiveName = false;
